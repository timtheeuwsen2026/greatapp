...
import { explainVenueDealMechanics, getVenueDealOptions } from "@shared/venueDealModels";
import { usePlatformFee } from "@/hooks/usePlatformFee";

/**
 * The internal partner tutorial.
 *
 * This page is the commercial model in full: every deal type with its exact
 * mechanics, the platform fee as a number, and the Deal Room negotiation flow
 * step by step. It used to be public at /how-it-works/partners, which meant a
 * competitor could read the entire structure of the business without so much
 * as an email address — and one has already lifted this product's positioning
 * once. So it now sits behind the same gate as the tooling it describes:
 * verified Creator, Venue and Promoter accounts only.
 *
 * What stayed public is a generic version at /how-it-works/partners, pitched
 * at the depth of the participant guide, whose job is to get someone to sign
 * up rather than to teach them the model for free.
 *
 ...
 * longer existed.
 */
export default function PartnerTutorial() {
          ...
          {deals.map((deal) => (
            <tr key={deal.value} className="border-b border-gray-100 dark:border-gray-800 align-top">
              ...
              <td className="py-3 pr-4 text-gray-600 dark:text-gray-400">
                <span className="block">{deal.description}</span>
                {/* The exact mechanic, in the same words the builder shows when
                    this deal is selected. Three of these — Venue Sponsorship,
                    Price Per Participant Package, Per Room / Per Night — had
                    only ever been inferred from their names, which is precisely
                    where two sides shake hands meaning different things. */}
    ...
    <div className="min-h-screen bg-white dark:bg-gray-950">
      ...
      <section className="pt-24 pb-12 px-4 sm:px-6 lg:px-8 text-center bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-950">
        <div className="max-w-3xl mx-auto">
          <Badge variant="secondary" className="mb-4">Partner tutorial</Badge>
      ...
      {/* Walkthrough video. Empty until Tim fills it in from the admin
          dashboard, and visibly empty rather than silently absent. */}
      <section className="px-4 sm:px-6 lg:px-8 pb-4">
        <div className="max-w-3xl mx-auto">
          <VideoEmbedSlot
            slot="partnerTutorialVideoUrl"
            title="Watch the walkthrough"
            description="The whole flow end to end — listing, matching, agreeing terms, getting paid."
          />
        </div>
      </section>
